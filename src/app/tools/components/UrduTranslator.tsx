...
export function UrduTranslator() {
  ...
  const [sourceLang, setSourceLang] = useState<Language>('Roman-Urdu');
  ...
  useEffect(() => {
    const handleTranslate = async () => {
      ...
      setIsLoading(true);
      try {
        const result = await translateUrduText({ text: debouncedInputText, sourceLanguage: sourceLang, targetLanguage: targetLang });
        ...
        <Accordion type="single" collapsible>
            <AccordionItem value="guide" className="border-none">
                ...
                <AccordionContent className="mt-2 p-6 rounded-lg bg-card/50 dark:bg-card/20 prose dark:prose-invert max-w-none text-sm">
                    <p>This tool allows you to translate and transliterate text between English, Urdu (Perso-Arabic script), and Roman-Urdu (Urdu written in Latin script) in real-time.</p>
                    <ol>
                        <li>Select the source ("From") and target ("To") languages. To use it as a typing trainer, set "From" to Roman-Urdu and "To" to Urdu.</li>
                        <li>Start typing in the left text box. The translated or transliterated text will appear automatically in the right box as you type.</li>
                        <li>Use the swap button (⇄) to quickly switch the languages and text.</li>
                    </ol>
                    <p>It's perfect for content creators, students, and anyone who needs to switch between different forms of Urdu and English for communication in Pakistan or wants to learn the Urdu script.</p>
                </AccordionContent>
            </AccordionItem>
        </Accordion>
      </div>
